...
function VerificaRUT(rutEnviado) {
  ...
  function validarRut() {
    // Obtén el valor actual del campo de entrada de RUT
    const rutInput = document.getElementById('rut');
    const rutValue = rutInput.value;

    // Crea una instancia de VerificaRUT con el valor del RUT
    const rutValidado = new VerificaRUT(rutValue);

    // Obtiene el elemento del mensaje de error
    const mensaje = document.getElementById('mensaje');

    // Verifica si el RUT es válido y muestra el mensaje correspondiente
    if (!rutValidado.valida()) {
        mensaje.innerHTML = "Digite un RUT válido";
    } else {
        mensaje.innerHTML = ""; // Limpia el mensaje si el RUT es válido
    }
}
